...
export function registerAppendMessages(
  ...
  server.registerTool(
    ...
    {
      description:
        "Save something to the user's long-term memory so it's available in every future session (in this app and any other AI they connect). CALL THIS PROACTIVELY, without being asked, whenever the user states a durable preference, fact about themselves or their work, a decision and its reasoning, a goal, or a project detail worth remembering later — that is the whole point of Engram, and the user set it up expecting you to remember on your own. Stores messages verbatim, auto-chunked + embedded for search. conversation_id is optional: omit it to append to the user's default memory, or set it to group messages under a specific conversation. The response returns the conversation_id used. Stores only the messages passed in — it cannot fetch past or external chat history (bulk import is separate, via the Engram CLI). Not for secrets (passwords, API keys, tokens, government IDs): stored text is searchable and resurfaces in future context; use Engram's separate zero-knowledge encrypted vault for credentials. Optionally accepts client-encrypted vault entries.",
      inputSchema: {
      conversation_id: z
        .string()
        .optional()
